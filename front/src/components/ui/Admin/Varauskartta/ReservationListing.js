  ...
  render() {
    ...
    for (let i = 0, j = this.props.items.length - 1; i < this.props.items.length; i++ , j--) {
      ...
      if (this.state.rows[i + 1]) {
        ...
        items.push(
          <TableRow key={i}>
            <TableRowColumn colSpan="4">{this.props.items[j].category} ({this.props.items[j].subCat})<br />Ilmoitettu: {this.parseTimeStamp(this.props.items[j].junkdateadded)}</TableRowColumn>
    ...
    return (
      <MuiThemeProvider>
        <div>
          ...
          <Dialog key={'i'} // if item has been reserved
